...
public class HandlerList {
    ...
    public static void unregisterAll(MCloud mCloud) {
        synchronized (allLists) {
            for (HandlerList h : allLists) {
                h.unregister(mCloud);
            }
    ...
    public synchronized void unregister(MCloud mCloud) {
        boolean changed = false;
        for (List<RegisteredListener> list : handlerSlots.values()) {
            for (ListIterator<RegisteredListener> i = list.listIterator(); i.hasNext(); ) {
                if(i.next().getMCloud().equals(mCloud)) {
                    i.remove();
                    changed = true;
                }
            }
    ...
    public static ArrayList<RegisteredListener> getRegisteredListeners(MCloud mCloud) {
                ...
                synchronized (h) {
                    for (List<RegisteredListener> list : h.handlerSlots.values()) {
                        for (RegisteredListener listener : list) {
                            if(listener.getMCloud().equals(mCloud)) {
